...
public:
        ...
        : m_bError(false)
    {
        ...
        m_roleNames.insert(role++, "page_url");
        m_roleNames.insert(role++, "playtimes_tag");
    ...
    void updateCurrentVideo(int row)
    {
        VideoData *d = m_videos[row];
        int role = 0;
        m_currentVideo.m_name = d->at(role++);
        m_currentVideo.m_date = d->at(role++);
        m_currentVideo.m_directorTag = d->at(role++);
        m_currentVideo.m_director = d->at(role++);
        m_currentVideo.m_actorTag = d->at(role++);
        m_currentVideo.m_actor = d->at(role++);
        m_currentVideo.m_ratingTag = d->at(role++);
        m_currentVideo.m_rating = d->at(role++);
        m_currentVideo.m_descTag = d->at(role++);
        m_currentVideo.m_desc = d->at(role++);
        m_currentVideo.m_poster = d->at(role++);
        m_currentVideo.m_pageUrl = d->at(role++);
        m_currentVideo.m_playtimesTag = d->at(role++);
        m_currentVideo.m_playtimes = d->at(role++).toUInt();
    }
...
VideoDetail * VideoListModel::currentVideo()
{
    return &m_dptr->m_currentVideo;
}

void VideoListModel::updateCurrentVideo(int row)
{
    m_dptr->updateCurrentVideo(row);
}

void VideoListModel::search(QString text)
{
    //TODO: impl
    qDebug() << "received search request: " << text;
}
